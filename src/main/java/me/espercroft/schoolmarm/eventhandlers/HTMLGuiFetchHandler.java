...
import me.espercroft.schoolmarm.HttpUserInterface;
...
public class HTMLGuiFetchHandler implements EventHandler {
    ...
    public Runnable createTask(Event evt) {
        Runnable task = new Runnable() {
            ...
            public void run() {
                try {
                    System.out.println("GUIFETCH EVENT"); //TODO REMOVEME
                    Event event = evt;
                    ...
                    switch (query) {
                        ...
                        case "master_size":
                            ...
                            send(exchange,response);
                            break;
                        case "overview_sections":
                            response.append(HttpUserInterface.listToJson(marm.getSectionIndex()));
                            System.out.println(response);
                            send(exchange,response);
                            break;
                        case "overview_teachers":
                            response.append(HttpUserInterface.listToJson(marm.getTeacherIndex()));
                            System.out.println(response);
                            send(exchange,response);
                            break;
                        case "master_index":
                            {
                            List<Student> masterIndexEntries = new ArrayList<>(marm.getMasterIndex().values());
                                response.append(HttpUserInterface.listToJson2(masterIndexEntries));
                            }
                            System.out.println(response);
                            send(exchange,response);
                            break;
    ...
    private static void send(HttpExchange exchange, CharSequence response) {
        System.out.println("Sending response"); //TODO REMOVEME
        System.out.println(response);
        try {
